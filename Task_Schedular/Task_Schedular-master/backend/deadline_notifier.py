...
class DeadlineNotifier:
    def __init__(self, excel_file_path: str, smtp_config: Dict):
        # Use a separate Excel file for notifier to avoid schema conflicts with API storage
    ...
    def send_email(self, to_email: str, subject: str, body: str) -> bool:
        try:
            if not self.smtp_config.get('sender_email') or not self.smtp_config.get('sender_password'):
                logging.error("SMTP credentials are not set. Skipping email send.")
                return False
            msg = MIMEMultipart()
...
def main():
    # Use a separate file so the notifier expects its own schema
    excel_file_path = "notifier_tasks.xlsx"
